...
// TEAPOT CONTROLS
float TBaseTransX = 0.5f;		// 0
float TBaseTransZ = 0;
float TBaseSpin = 0;		// 1
float TShoulderAng = 0;		// 2
float TElbowAng = 0;
float TWristAng = 0;		// 3
float TWristTwistAng = 0;
...
void myDisplay()
{
	...
	objectXform = glm::rotate(objectXform, glm::radians(TBaseSpin), glm::vec3(0.0f, 1.0f, 0.0f));
	...
	objectXform = glm::rotate(objectXform, glm::radians(WristTwistAng), glm::vec3(0.0f, 1.0f, 0.0f));
	objectXform = glm::rotate(objectXform, glm::radians(TWristTwistAng), glm::vec3(0.0f, 1.0f, 0.0f));
	objectXform = glm::rotate(objectXform, glm::radians(-WristTwistAng), glm::vec3(0.0f, 1.0f, 0.0f));
	objectXform = glm::rotate(objectXform, glm::radians(TWristAng), glm::vec3(0.0f, 0.0f, 1.0f));
	objectXform = glm::rotate(objectXform, glm::radians(-WristAng), glm::vec3(0.0f, 0.0f, 1.0f));
	objectXform = glm::translate(objectXform, glm::vec3(0.0f, -0.5f, 0.0f));

	objectXform = glm::rotate(objectXform, glm::radians(TElbowAng), glm::vec3(0.0f, 0.0f, 1.0f));
	objectXform = glm::rotate(objectXform, glm::radians(-ElbowAng), glm::vec3(0.0f, 0.0f, 1.0f));
	objectXform = glm::translate(objectXform, glm::vec3(0.0f, -0.5f, 0.0f));

	objectXform = glm::rotate(objectXform, glm::radians(TShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f));
	objectXform = glm::rotate(objectXform, glm::radians(-ShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f));
	objectXform = glm::translate(objectXform, glm::vec3(0.0f, -0.4f, 0.0f));

	objectXform = glm::translate(objectXform, glm::vec3(TBaseTransX, 0.0f, TBaseTransZ));
	objectXform = glm::translate(objectXform, glm::vec3(-BaseTransX, 0.0f, -BaseTransZ));
	...
	model2 = glm::rotate(model2, glm::radians(-FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(model2);

	// Right Finger Tip
	model2 = glm::translate(model2, glm::vec3(0.0f, 0.35f, 0.0f));
	model2 = glm::rotate(model2, glm::radians(-FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		PhongShader->setVec3("position", camera.Position);
		PhongShader->setVec3("direction", camera.Front);
		PhongShader->setBool("angularAttenuation", AngularAttenuation);
		...
		FloorShader->setVec3("position", camera.Position);
		FloorShader->setVec3("direction", camera.Front);
		FloorShader->setBool("angularAttenuation", AngularAttenuation);
...
void setupShader()
{
	...
	PhongShader->setFloat("cutOff", glm::cos(glm::radians(12.5f)));
	PhongShader->setFloat("outerCutOff", glm::cos(glm::radians(17.5f)));

	PhongShader->setFloat("constant", 1.0f);
	PhongShader->setFloat("linear", 0.09f);
	PhongShader->setFloat("quadratic", 0.032f);

	FloorShader = new Shader("advanced_lighting.vs", "advanced_lighting.fs");
	FloorShader->use();
	FloorShader->setVec3("lightColor", lightColor);

	FloorShader->setFloat("cutOff", glm::cos(glm::radians(12.5f)));
	FloorShader->setFloat("outerCutOff", glm::cos(glm::radians(17.5f)));

	FloorShader->setFloat("constant", 1.0f);
	FloorShader->setFloat("linear", 0.09f);
	FloorShader->setFloat("quadratic", 0.032f);
}
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (LeftButtonDown)
	{
		switch (RobotControl)
		{
		case 0: BaseTransX += xoffset; BaseTransZ -= yoffset; 
			if (TeapotControl) {
				TBaseTransX += xoffset; TBaseTransZ -= yoffset;
			}
			break;
		case 1: BaseSpin += xoffset * 180;
			if (TeapotControl) {
				TBaseSpin += xoffset * 180;
			}
			break;
		case 2: ShoulderAng += yoffset * -90; ElbowAng += xoffset * 90;
			if (TeapotControl) {
				TShoulderAng += yoffset * -90; TElbowAng += xoffset * 90;
			}
			break;
		case 3: WristAng += yoffset * -180; WristTwistAng += xoffset * 180;
			if (TeapotControl) {
				TWristAng += yoffset * -180; TWristTwistAng += xoffset * 180;
			}
			break;
